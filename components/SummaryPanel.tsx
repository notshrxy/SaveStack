                ...
                <form onSubmit={handleSendMessage} className="flex gap-3">
                  <div className="relative flex-grow">
                    <input
                      ...
                      placeholder={isSending ? "" : "Type a question..."}
                      disabled={isSending}
                      className="w-full p-4 border-4 border-black rounded-none font-black text-sm focus:outline-none focus:bg-yellow-50 transition-colors bg-[#FAF5E9] disabled:opacity-50"
                    />
                    {isSending && (
                      <div className="absolute left-4 top-1/2 -translate-y-1/2 flex gap-1.5 items-center pointer-events-none">
                        <div className="w-2 h-2 bg-black rounded-full animate-pulse"></div>
                        <div className="w-2 h-2 bg-black rounded-full animate-pulse [animation-delay:200ms]"></div>
                        <div className="w-2 h-2 bg-black rounded-full animate-pulse [animation-delay:400ms]"></div>
                        <span className="ml-2 text-xs font-black uppercase tracking-widest opacity-50">Genie is thinking...</span>
                      </div>
                    )}
                  </div>
                  <button type="submit" disabled={isSending || isLoading || !userInput.trim()} className="bg-black text-white p-4 rounded-none border-4 border-black hover:bg-purple-600 transition-all disabled:opacity-30 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
